...
- The pretty layer now includes friendlier archetype names, show-off stats, a shareable scoreboard, comparative slices, victory-lap sessions, and deterministic badges so the results are easier to read outside an engineering context.
...
- Comparative slices summarize the selected corpus plus recent windows like `Recent 100`, `Recent 500`, and `Recent 1000` when enough sessions exist. The report derives a human-facing momentum section from those slices instead of persisting a separate momentum array.
...
- Recent momentum in the report deliberately prefers a stabler recent window when available, so the headline trend readout is less twitchy than a tiny-slice comparison.
